...
/**
  * 一小时内登录失败次数
  * (JAVA)时间约束:截止当前时刻为止,时间范围一个半小时，以半个小时的步长推进
  * (Scala) window:[长度1小时,推进频率1分钟]
  */
object HourLoginFail {
    def loginFair(ssc: StreamingContext,  streamData: InputDStream[ConsumerRecord[String, String]],data:DStream[LoadLogInfo]): Unit = {
        //计算单批次查询次数
        var pcMap: Map[String, Int] = Map()
        //获取系统阈值
        val thresholdValue: Int = GlobalConfig.threshold.getOrElse("HourLoginFail", 10)
        ...
        json.transform(x => {
            ...
            .map(x => {
                //获取历史记录
                val userTuple3 = ViolationsRepository.Repository.get("HourLoginFail").
                    get.getOrElse(x._1, Tuple3(0, "", x._9 + "_" + x._3)) //(累计次数,最后一次操作时间)
                if (userTuple3._1 >= thresholdValue & x._9 == userTuple3._2) {
                    //更新标记
                    var historyMap = ViolationsRepository.Repository.get("HourLoginFail").
                        get.updated(x._1, (0, x._9, x._9 + "_" + x._3))
                    ViolationsRepository.Repository = ViolationsRepository.Repository.updated("HourLoginFail", historyMap)
                    Row(Nil)
                } else {
                    if (x._20 == 0 & x._9 > userTuple3._2) {
                        ...
                        if (x._20 <= userTuple3._1 || x._9 <= userTuple3._2) { //不要的
                            ...
                            ViolationsRepository.Repository = ViolationsRepository.Repository.updated("HourLoginFail", historyMap)
                            println("更改后的时间" + ViolationsRepository.Repository.get("HourLoginFail"))
                            Row(x._1, x._2, x._3, x._4, x._5, x._6, x._7, x._8, x._9, x._10, x._11,
            ...
            .foreachRDD(rdd => {
                rdd.foreachPartition(it => {
                    ...
                    val ppst = conn.prepareStatement("insert into biz_log_hourloginfail(" +
